...
int main() {
  // Calling the getUserInput function and storing the return value in
  // numOfTerms
  ...
  if (numOfTerms < 0) {
    ...
    // Initialised with 1, cause the expansion starts with 1 + 1/1! + 1/2! +
    // 1/3! + ... + 1/n! In a sense this is accounting for the fact that 0! = 1.
    double exp_x{1};
    ...
    // Printing the result
    std::cout << "The Taylor Series Approximation of exp(x) upto " << numOfTerms
              << " Terms is: " << exp_x << '\n';
