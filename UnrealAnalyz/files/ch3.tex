...
\defn{Norm}{Given an inner product space $V(\FF), \langle \cdot \rangle$, we define the \emph{norm} or \emph{length} of a vector $x$, given by $|| x||$ as $\sqrt{\langle x|x \rangle}$ }
...
\defn{$l_p$-Space}{Define $l_p$ as the space of all sequences $(a_n)_{n=1}^{\infty}$ such that $\sum_{i=1}^{\infty} |a_i|^p$ exists.}

It is easily seen thet $l_p$ is a vector space over $\RR$. We defne the metric on $l_p$ as $d_p(\{x\},\{y\})=(\sum_{i=1}^{\infty}|x_i-y_i|^p)^{1/p}$
