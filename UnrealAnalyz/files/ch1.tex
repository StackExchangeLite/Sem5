...
Now suppose $z_i=\frac{|x_i|}{\sum_{i=1}^k |x_i|^p}$ and $w_i=\frac{|y_i|}{\sum_{i=1}^k |y_i|^q}$. It is easy to see that $\sum_{i=1}^k |z_i|^p=\sum_{i=1}^k|w_i|^p=1$. This means, from Young's inequality, $$\sum_{i=1}^k |z_i w_i|\leq 1 $$ which implies $$\sum_{i=1}^k |x_iy_i|\leq (\sum_{i=1}^k |x_i|^p)^{1/p} (\sum_{i=1}^k |y_i|^q)^{1/q} $$
