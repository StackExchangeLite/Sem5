...
\thmp{Generalised AM-GM}{If $x,y \in \RR^{+}$ and $t\in(0,1)$, then $$(1-t)x+ty \geq x^{1-t}y^{t} $$}{}
...
\thmp{Hoelder's Inequality}{Given $\{x_i\}_{i=1}^k, \{y_i\}_{i=1}^k$ in $\RR$, we have (given positive integers $p,p$)
$$\sum_{i=1}^k |x_i y_i| \leq (\sum_{i=1}^k |x_i|)^{1/p} (\sum_{i=1}^k |y_i|^q)^{1/q}$$
}{}
\thmp{Minkowski's Inequality}{}{}
\newpage
...
\thmp{Real numbers in $[0,1]$ are equivalent to the set of all $0,1-$sequences (i.e, sequences containing only $1$ or $2$) (\textbf{p-adic proof})}{We see that for }{Consider a point $x$ in $[0,1]$. By virtue of the above decmial expansion results, we can find a sequence $a_n$ consisting of just $0$ and $1$ so that $\sum_{i=1}^{\infty} \frac{a_n}{2^n}$ converges to $x$. For every $0,1$ sequence, we can map a unique number in $[0,1]$ so that that sequence becomes the $2-$adic or binary representation of that number. This is a well defined surjection from the set of all $0,1$ sequences to $[0,1]$. }
\end{document}
