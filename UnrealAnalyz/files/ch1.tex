...
\begin{center}
\emph{"In order to make apple pie from scratch, you must first invent the universe."-Carl Sagan}
{Here we take up precisely that endeavour.
}
\end{center}
...
\defn{Maximal element of a poset $P$}{An element $m \in P$ is maximal if there is \emph{no element of } $P$ that greater than $m$. i.e, either an element $q$ is such that $q \leq m$ or $q$ is not comparable to $m$}
\defn{Upper bound of subset $S$ of poset $P$}{An element $U$ of $P$ is an upper bound for $S$ if $U$ is larger than or equal to \emph{every element} of $S$. }
...
\asum{Zorn's Lemma}{Let $P$ be a partially ordered set such that $P$ is non empty and every totally ordered subset has an upper bound in $P$. Then, $P$ has a maximal element (atleast one)}

\thmp{}{AC $\iff$ Zorn's Lemma}{hmm}

\textbf{Some Corollaries of AC:}
\thmp{}{Every vector space has a basis}{Let $L$ be a linearly independent set of $V$. Consider $A$, the collection of all linearly independent sets that contains $L$. This set is partially ordered by inclusion. Let $C$ be a chain of sets in $A$. $\cup C$, then, would be a linearly independent set for if not, there would exist a finite subset of $\cup C$ such that its linear combination would be 0, but not all coefficients would be. That means, the linearly independent set that contains all these finite vectors would turn out to be dependent. Hence, we have shown that every chain has an upper bound $\cup C$. Hence, from Zorn's lemma, $A$ has a maximal element $M$. M is, ofcourse, linearly independent. Suppose $M$ does not span $V$. That means that there is an element $\beta \in V$ that is not in $span\{M\}$, which means that $M \cup\{\beta\}$ would be a linearly independent set that is larger than the maximal $M$. Contradiction. Hence, $V$ has a basis.}
...
\newline \textbf{An ALT PROOF idea:} The setup is the same as before up until where we supposed that $u^n<y$. We want to find a $h$ such that $f(u+h)<y$. We prove this by proving the continuity of $f$. $f(u+h)-f(u)$(note that $f$ is monotone increasing)$=(u+h)^n-u^n=(h)(u^{n-1}+u^{n-2}(u+h)+ \cdots (u+h)^{n-1})\leq n(h)(u+h)^{n-1}$. Choose $h$ smaller than $1$ to get $f(u+h)-f(u)\leq nh (u+1)^{n-1}$. Now we see that $f$ is actually lipschitz.  }
...
\lemp{}{Let $p\geq 2$ be a fixed integer and $0<x<1$. If $x$ has finite length decimal expansion, then $x$ has precisely two decimal expansions. If $x$ does not have a finite length decimal expansion, then $x$ has a unique decimal expansion.}{Let $x$ have a finite length decimal expansion and let $$x=a_1/p+a_2/p^2+\cdots a_k/p^k+0/p^{k+1}+\cdots$$
Suppose that $a_k=(a_k-1)+1$ and let $a_k'=a_k-1$ possibly in the range of $[0,p-2]$. Then we have $$x=a_1/p+a_2/p^2 \cdots a_k'/p^k+ 1/p^k +0/p^{k+1}+ 0/p^{k+2} \cdots $$
It is at this point that we recall a simple fact: $$1/p^k+0/p^{k+1}_0/p^{k+2}\cdots =1/p^{k}= \sum_{i=k+1}^{\infty} (p-1)/p^i $$
Thus, we see that $1/p^k+0/p^{k+1}\cdots$ is equal to $0/p^k+(p-1)/p^{k+1}\cdots $ and hence $$x=a_1/p+a_2/p^2 \cdots a_k'/p^k+1/p^k+0/p^{k+1}\cdots =$$ $$a_1/p+a_2/p^2 \cdots a'_k/p^k + 0/p^k +(p-1)/p^{k+1}\cdots $$ So we have established another decimal expansion for our $x$.  
\\\\
Now we try to characterise the situation about uniqueness: Let $x=a_1/p^1+a_2/p^2+\cdots$ and also suppose $x=b_1/p^1+b_2/p^2+\cdots$. Suppose (without loss of generality) they differ from the first position [If they don't, we find the first position where they do differ, say $k$, so we multiply $p^{k-1}$, and subtract the two, and the new number we get is essentially 0, with the first decimal place being (supposedly) non zero].
\\\\ $a_1\neq b_1$. Say $a_1<b_1$, which gives us $x-x=(b_1-a_1)/p^1+ (b_2-a_2)/p^2 \cdots $ which could be rewritten (with a change of notation) as $0=q_1/p^1+q_2/p^2 \cdots $ where $q_i=b_i-a_i$. Note that $q_1\geq 1$. This means that, in order for the whole sum to be $0$, we require $q_2/p^2+q_3/p^3 \cdots$ to be negative, and equal to $-q_1/p$. 
This amounts to saying $$-q_1=q_2/p+q_3/p^2 \cdots$$
From here, a few facts are clear: Since $q_1\geq 1$, we have $q_2/p+q_3/p^2 \cdots = -q_1 \leq -1$. Note that, since $-(p-1)\leq -a_i \leq 0$ and $0\leq b_i \leq p-1$, we have that $-(p-1)\leq q_i \leq p-1$. So we see that, speaking "absolutely", i.e, $|q_2|/p+|q_3|/p^2 .... \leq 1$ (since $|q_i|$ are all less than $p-1$). Therefore, $|q_1|\leq |q_2|/p+|q_3|/p^2 \cdots\leq 1$. Hence, the difference $q_1$ cannot exceed 1. But since we assumed it is larger than or equal to 1, we must have that $q_1=1$. Now we may proceed. We have $1=q'_2/p+q'_3/p^2 \cdots$ where $q'_i=-q_i=a_i-b_i$ (These too, follow the same bounds). Note that, if even one of our $q_i'<p-1$, i.e, say for some $k$, $q'_k\leq p-2-2,$ then the sum cannot be equal to $q$. We will prove this: 
\\\\ Assume at $k+1$, we have $q'_{k+1}<p-1$, then $$q'_2/p+q'_3/p^2 \cdots \leq (p-1)/p+(p-1)/p^2 + \cdots (p-1)/p^{k-1}+ (p-2)/p^{k}+ (p-1)/p^{k+1} \cdots $$ We split this sum into 2 parts: $L_1 + (p-2)/p^{k}+L_2$ where $L_1= (p-1)(1/p+1/p^2 \cdots 1/p^{k-1})$ and $L_2=(p-1)(\sum_{i=k+1}1/p^i)=1/p^k$. Evaluation of $L_1$ is likewise not hard: $(p-1/p)(1+1/p+1/p^2+\cdots 1/p^{k-2})= (p-1/p)(1-1/p^{k-1})/(1-1/p)=1-1/p^{k-1}$. So, $L_1+L_2+(p-2)/p^k$ gives us $$1-1/p^{k-1}+ (p-2)/p^k + 1/p^k=1-1/p^{k-1}+(p-1)/p^k<1$$ (strictly). Therefore, we see that all our $q'_i$ must be equal to $p-1$, which means that $a_i-b_i=p-1 \implies a_i=p-1+b_i$. Since both $a_i$ and $b_i$ are to be bounded above and below (resp.) by $p-1$ and $0$, we must have that the only instance where it is possible for $q'_i=p-1$ is that $a_i=p-1$ and $b_i=0$. Therefore, we have shown that if under the consideration that we do have 2 different decimal expansions, it ought to be the case that one of the expansions must be eventually $0$, and the other consisting of $p-1$ trailing. So if $x $ does not have a finite length expansion, and given that we've seen that two expansions ought to differ by a series $\{q'_i\}$ where each $a_i=b_i+p-1$, we cannot have an instance where any $b_i$ exceeds $0$. Hence, we have shown that if $x$ does not have a finite length expansion, it has a unique expansion.
}
...
\lemp{}{$[0,1) \equiv [0,1) \times \{y,n\}$}{One side injection is trivial ($[0,1) \times \{y,n\}$ to $[0,1)$ is obvious). Consider the map $g:[0,1) \to [0,1)\times \{y,n\}$ given by considering only those binary expansions of $x$ that don't have trailing 1s: $$g(0.x_0x_1x_2 \cdots)= \begin{cases}
(0.x_1x_2 \cdots,n) \text{ if } x_0=0 \\ (0.x_1x_2 \cdots,y) \text{ if } x_0=1
\end{cases}$$ This is obviously a surjection. Hence, from AC, we can conclude that there exists an injection from $[0,1) \times 
\{y,n\}$ into $[0,1)$. Hence, from $CSB$, we are done.}
...
\thmp{A strenghtening of the previous result}{ The space $\{0,1\}^{\omega}$ of all 0,1 sequences, is equivalent to the space $[0,1)$ (\textbf{We shall assume CSB theorem})}{Consider the obvious injection from $[0,1)$ into $\{0,1\}^{\omega}$ by considering the unique binary expansions of numbers neglecting those with trailing 1s. 
\\\\ For an injection from $\{0,1\}^{\omega}$ to $[0,1)$, we first consider an injection from $\{0,1\}^{\omega}$ to $[0,1) \times \{y,n\}$, given by $g(x_1,x_2,x_3\cdots)=(0.x_1x_2 \cdots,y \text{ or } n) $, y or n depending on whether the sequence has a trailing 1s or not. This is again an injection. We are done.}
...
\thmp{(Partial Result reg |S|=|{set of all finite subsets of }S|)}{$|\NN|=$|Set of all finite subsets of $\NN$| (Assumed use of cantor shroeder berstein)}{First we consider the set $W:=\{(x_1,x_2,\cdots): x_i \in \NN, (x_i)_{i\in \NN}\text{ eventually zero}\}$
We first order all the primes $p_1<p_2<\cdots$ and define a map $(x_1,x_2,x_3,\cdots)\mapsto \prod_{i=1}^{\infty} p_i^{x_i}$. This map happens to be a bijection:
To see that it is an injection, let $$\prod_{i=1}^{\infty} p_i^{x_i}=\prod_{i=1}^{\infty} p_i^{y_i}$$
From fundamental theorem of arithmetic, we see that $x_i=y_i$ and hence $(x_i)_{i\in \NN}=(y_i)_{i\in \NN}$, showing the map is injective.
Surjection is also obvious from the fundamental theorem, as every natural number has a unique prime decomposition of the form $p_1^{x_1} p_2^{x_2}\cdots$. \textbf{Note:} \emph{We have taken for convenience that 0 is a Natural Number. Also note that the map is well defined precisely since we restrict the sequences be eventually 0.}
We now see $$\NN \leftrightarrow |W|$$
But there is an obvious injection from the set of all finite subsets of $\NN$ (denoted $FS$) into $W$, as follows: $$ f: FS \hookrightarrow W $$ given by (first ordering our elements) $$\{x_1,x_2\cdots x_n\}\mapsto (x_1,x_2\cdots x_n,0,0,0 \cdots) $$
Hence we see that $|FS| \leq |W|=|\NN|$. Moreover, it is easy to see that $|\NN|\leq |FS|$. Hence $|\NN|=|FS|$
\\\\ \textbf{Alt Idea (Slicker):}
\\ 
We can also directly define a map from $FS \hookrightarrow \NN$ as follows: 
Let $A$ be a finite subset of $\NN$. $A=\{x_1,x_2,\cdots x_n\}$ where $x_i$ are distinct (ascendingly ordered) natural numbers. We send $A$ to $A \mapsto p_{x_1}p_{x_2}\cdots p_{x_n}$ which is a squarefree natural number. To see that this is injective, we note that if $p_{x_1}p_{x_2}\cdots p_{x_n}=p_{y_1}p_{y_2}\cdots p_{y_m}$, from fundamental theorem of arithmetic, we must have that $p_{x_i}=p_{y_i}$ as well as $n=m$. Hence, we have a direct injection. 
}

\section{Can we do single variable calculus real quick?}

\subsection{One variable continuous functions}
\exm{An example:}{}
