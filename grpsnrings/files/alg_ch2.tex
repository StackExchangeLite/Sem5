...
\\\\ $\impliedby$) Suppose the only subgroups of $G$ are the trivial one and itself. Suppose $G$ is of a non prime order $q$. Then, if $m | q $, there is an element $a^{q/m}$, and its corresponding generated set, that is a proper subgroup of $G$. Absurd. }
\thmp{Cyclic Cauchy}{Let $G=\langle a \rangle$ be a finite group of order $n$. Let $m | n$. Then, there exists a unique subgroup (that is obviously cyclic) whose order is $m$}{Let $m | n$. Consider $a^{n/m}$. $O(a^{n/m})=O(\langle a^{n/m}\rangle)=m$. Existence is clear. Let $a^j$ be another element whose order $|a^j|=n/gcd(j,n)=m$. Note that, $n/m=gcd(j,n)$ implying $n/m$ divides $j$, making $a^j$ part of $\langle a^{n/m}\rangle$ which means $\langle a^j \rangle \leq \langle a^{n/m} \rangle$. (This means that they are indeed the same group).}
\newpage
...
\\\\ \textbf{(Aliter)} An easier way to see that there would exist no $r$ smaller than $m/gcd(i,m)$ such that $\sigma^{ri}(a)=a$ (or $ri\equiv 0 mod(m)$), is that, if so, $ri=\gamma \cdot m$ which means $r (i/gcd(i,m))=\gamma \cdot (m/gcd(i,m))$ which means (since $r$ is smaller than $m/gcd(i,m)$) $m/gcd(i,m)$ must divide $i/gcd(i,m)$, which is impossible since they share no common factors other than $1$. }
