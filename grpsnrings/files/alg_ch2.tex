...
\thmp{}{A subgroup $H$ is normal to $G$ (denoted $H \triangleleft G$)if and only if $N_G(H)=G$.}{}
...
\\\\ Note that the collection of distinct equivalence classes form a partion of the entire group $G$. If $G$ is finite, say, of order $n$, then the sum of the cardinalities of all the equivalce classes ought to give us $n$. Formally this means, suppose $X=\{[a]:a\in G\}$ be the collection of all (distinct) equivalence classes. Then $$\sum_{[a] \in X}|[a]|=|G|$$ But since $|[a]|=|aH|=|H|$, we can rewrite the above as:
$$\sum_{i=1}^{|\{[a]: a \in G\}|}|H|=|G| $$
which implies $$|\{[a]:a \in G \}|\cdot|H|=|G| $$
