...
\end{enumerate}}{(1) If $d$ is the GCD of $a,b$, obviously $d|a$ and $d|b$ $\implies dz_a=a, dz_b=b$ for some $z_a,z_b \in \ZZ$. Consider an arbitrary divisor $q$ such that $\exists r_a,r_b \in \ZZ$ such that $qr_a=a,qr_b=b$. Therefore, $dz_a=qr_a$ and $dz_b=qr_b$ $\implies \frac{dz_a}{q}=r_a$ and $\frac{dz_b}{q}=r_b$. Now either $q$ divides $d$, or $q$ divides $z_a$ and $z_b$ which means there would be $\delta_a$ and $\delta_b$ so that $q\delta_a=z_a$ and $q\delta_b=z_b$. If thats the case,
then $dq\delta_a=a$ and $dq\delta_b=b$. Hence, $dq$ is a divisor of $a$ and $b$, which makes it larger than $d$. This can't be, since $d$ is the largest such divisor. From here, uniqueness is clear.
\\\\(2) If $l$ is the LCM of $a$ and $b$, we have $ag_a=l$ and $bg_b=l$ for integers $g_a$ and $g_b$. Suppose that $m$ is another multiple of $a$ and $b$ but $l$ doesn't divide $m$. Meaning, there exists integers $y_a$ and $y_b$ so that $ay_a=m$ and $by_b=m$. This means $ly_a=mg_a$ and $ly_b=mg_b$. Since $l \leq m$, we have $y_a=\frac{mg_a}{l}$ and $y_b=\frac{mg_b}{l}$ and that $l$ divides $g_a$ and $g_b$. If $l$ divides $g_a$ and $g_b$, we would have $a\frac{g_a}{l}=1$ and $b\frac{g_b}{l}=1$ which is absurd. Hence, $l$ has to divide $m$. Uniqueness is clear from here. 
...
and $$b=r_{n_0+1}(\text{something}_2) $$ with the added fact that $r_{n_0+1}$ divides every remainder $r_n$ in the division algorithm performed with $a$ and $b$. \end{document}
